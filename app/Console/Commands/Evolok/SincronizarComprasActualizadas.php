...
class SincronizarComprasActualizadas extends EvolokSyncCommand
{
    protected $signature = 'evolok:compras-actualizadas {fecha? : Fecha operativa a reprocesar (Y-m-d). Por defecto, ayer}';
    protected $description = 'Importa a BigQuery las compras de Evolok modificadas en el día operativo anterior (o en la fecha indicada)';
    ...
    public function handle(EvolokClient $client): int
    {
        ...
        $query = [
            '$or' => [
                [
                    '$and' => [
                        ['pm.lastModifiedDate' => ['$gte' => ['$date' => $fechaInicio], '$lt' => ['$date' => $fechaFin]]],
                        ['ic' => ['$exists' => true]],
        ...
        try {
            ...
            Log::error('evolok:compras-actualizadas falló', ['error' => $e->getMessage()]);
            return self::FAILURE;
        }
        ...
        $this->insertRows('Compras_Update', $compras);
